    ...
    <body>
      ...
      <div class="grid">
        <div class="box">
          ...
          <p><strong>${order.user?.name || order.shippingAddress?.fullName || 'Invité'}</strong></p>
          <p class="label">Email</p><p>${order.user?.email || order.guestEmail || '—'}</p>
          <p class="label">Téléphone</p><p>${order.user?.phone || order.guestPhone || order.shippingAddress?.phone || '—'}</p>
        </div>
        <div class="box">
              ...
              <>
                <tr key={o._id} className="hover:bg-gray-50 cursor-pointer" onClick={() => setExpanded(expanded === o._id ? null : o._id)}>
                  ...
                  <td className="px-5 py-3">
                    <p className="font-medium">{o.user?.name || o.shippingAddress?.fullName || 'Invité'}</p>
                    <p className="text-gray-400 text-xs">{o.user?.email || o.guestEmail || o.user?.phone || o.guestPhone || '—'}</p>
                    {!o.user && <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full">Invité</span>}
                  </td>
                  <td className="px-5 py-3 font-bold">{formatPrice(o.totalPrice)}</td>
