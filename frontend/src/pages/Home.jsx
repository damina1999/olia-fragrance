        ...
        <section className="relative overflow-hidden bg-dark-900">
          <div className="relative h-64 md:h-96">
            {events.map((ev, i) => (
              ...
              >
                {ev.image ? (
                  <img src={ev.image} alt={ev.title} loading="lazy" decoding="async" className="w-full h-full object-cover" />
                ) : (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {[
            ...
            <Link key={cat.value} to={`/products?category=${cat.value}`}
              ...
              <img src={cat.img} alt={cat.label} loading="lazy" decoding="async" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
