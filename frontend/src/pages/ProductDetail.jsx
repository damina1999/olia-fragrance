...
import { FiHeart, FiThumbsDown, FiShoppingCart, FiMinus, FiPlus, FiShare2 } from 'react-icons/fi';
      ...
      <div className="grid md:grid-cols-2 gap-10">
        ...
        <div>
          <div className="rounded-lg overflow-hidden mb-4">
            <img
              src={cloudinaryUrl(product.images?.[activeImg] || product.images?.[0] || 'https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?w=800&q=80', { w: 900, q: 80 })}
              alt={product.name}
              fetchpriority="high"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = 'https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?w=800&q=80';
              }}
          ...
          {product.images?.length > 1 && (
            <div className="flex gap-2">
              {product.images.map((img, i) => (
                ...
                >
                  <img
                    src={cloudinaryUrl(img, { w: 160, q: 70 })}
        ...
        <div>
          ...
          <div className="flex items-baseline gap-3 mb-5">
            ...
            {currentOldPrice && <span className="text-xl text-gray-400 line-through">{formatPrice(currentOldPrice)}</span>}
            {currentOldPrice && (
          ...
          {/* Social Share / Organic Boost */}
          <div className="mt-6 pt-5 border-t border-gray-100">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3 flex items-center gap-1.5">
              <FiShare2 className="text-gold-500" /> Partager ce parfum (Boost Gratuit)
            </p>
            <div className="flex flex-wrap items-center gap-2">
              <a
                href={`https://api.whatsapp.com/send?text=${encodeURIComponent(`Découvre ce parfum d'exception "${product.name}" chez Olia Fragrance 🌸✨ : ${window.location.href}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-medium transition shadow-sm"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-medium transition shadow-sm"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-medium transition"
              >
                <FiShare2 className="text-base" /> Copier le lien
              </button>
            </div>
          </div>
        </div>
