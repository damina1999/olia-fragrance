...
async function seed18Products() {
  try {
    await mongoose.connect(process.env.MONGO_URI);
    console.log('✅ Connecté à MongoDB');

    // Delete existing products & orders as requested
    await Product.deleteMany({});
    await Order.deleteMany({});
    console.log('🗑️ Produits et commandes nettoyés de la base de données');

    // Insert new 18 products
    const inserted = await Product.insertMany(customProducts);
    console.log(`🎉 ${inserted.length} nouveaux produits insérés avec succès !`);

    inserted.forEach((p, idx) => {
      console.log(`${idx + 1}. ${p.name} (${p.brand}) - 50ml: 39.9 DT / 30ml: 29.9 DT`);
    });
